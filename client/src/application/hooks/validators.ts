...
export const minLength = (length: number) => (value: string) => {
    return value.length >= length ? undefined : `Peab olema vähemalt ${length} tähemärki pikk`;
};
...
export const dateInFuture = (value: string) => {
    ...
    return inputDateOnly >= todayOnly ? undefined : 'Kuupäev peab olema tulevikus';
}
...
export const validateEstonianIdCode = (idCode: string): string | undefined => {
    ...
    let checksum = digits
        .slice(0, 10)
        .reduce((sum, digit, index) => sum + digit * weightsFirstRound[index], 0) % 11;

    if (checksum === 10) {
        checksum = digits
            .slice(0, 10)
            .reduce((sum, digit, index) => sum + digit * weightsSecondRound[index], 0) % 11;
    }
