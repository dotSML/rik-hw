...
export function EventOverview({
  ...
  handleSetData: (data: EventModel[]) => void;
  heading: string;
  className?: string;
  data: EventModel[];
  actions?: ReactNode;
  ...
  return (
    <div
      className={`flex flex-col items-center w-full bg-red h-full shadow-md ${className}`}
    >
      ...
      <div className="flex flex-col p-4 h-full w-full">
            ...
            ? data?.map((item, idx) => {
                return (
                  <EventListItem
                    key={`heading-${idx}`}
                    index={idx}
                    handleRemove={(eventId: string) => handleRemove(eventId)}
                    data={item}
                  />
                );
              })
        ...
        <div className="flex self-start">{actions}</div>
      </div>
    </div>
  );
}
