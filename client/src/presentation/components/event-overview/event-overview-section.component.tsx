...
export const EventOverviewSection = ({ upcomingEvents, pastEvents }: { upcomingEvents: EventModel[], pastEvents: EventModel[] }) => {
    return <div className="flex max-w-full w-full h-[20rem] my-4">
        <EventOverview heading="Tulevased üritused" data={upcomingEvents} className="basis-[50%] mr-4 shrink-0" actions={<button className="font-bold text-xs text-textPrimary">LISA ÜRITUS</button>} />
        <EventOverview heading="Toimunud üritused" data={pastEvents} className="basis-[50%]" />
    </div>
}
