...
export function AddAttendeeForm({
  ...
  handleSetValues: (data: Record<string, string>) => void;
  ...
  const naturalPersonFields = [
    ...
    { name: 'lastName', label: 'Perekonnanimi' },
    { name: 'personalIdCode', label: 'Isikukood' },
    {
      name: 'paymentMethodId',
      label: 'Maksemeetod',
      type: 'select',
      options: mapPaymentMethodOptions(paymentMethods),
    },
    {
      name: 'additionalInfo',
      label: 'Lisainfo',
  ...
  return (
    <form>
      <div className="grid grid-cols-3 gap-4">
        ...
        <RadioField
          name="type"
          className="col-span-2 gap-16 py-4"
          options={[
            { label: 'Eraisik', value: 'NaturalPerson' },
            { label: 'Ettevõte', value: 'LegalEntity' },
          ]}
          selectedValue={values.type}
          onChange={(val) => handleSetValues({ type: val })}
        />
      </div>

      {(values.type === AttendeeType.NaturalPerson
        ? naturalPersonFields
        : legalEntityFields
      ).map((field, idx) => {
        return (
