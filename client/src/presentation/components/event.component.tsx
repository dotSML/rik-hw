...
function AttendeeItem({ data, index, handleRemoveAttendee }: { handleRemoveAttendee: any, data: AttendeeModel, index: number}) {
    ...
    return <div className="flex flex-1 justify-between items-center whitespace-nowrap gap-4">
        ...
        <div className='flex'>
            ...
            <Button variant="text" onClick={() => handleRemoveAttendee(data.id)} title="KUSTUTA" />
        </div>
    </div>
}

export function EventDetails({ eventData, attendees, handleSetAttendees }: { eventData: EventModel, attendees: AttendeeModel[], handleSetAttendees: (attendees: AttendeeModel[]) => void }) {
    const handleRemoveAttendee = (attendeeId: string) => {
        deleteAttendee(attendeeId).then((result) => {
            if (result) {
                handleSetAttendees((attendees) => attendees.filter((a) => a.id !== attendeeId))
            }
        })
    }
    return <div>
